...
renderJQuery root = iterM go
  ...
  go (When wd) = runWhenData wd (\pred onTrue onFalse k -> do
    let render f = renderJQuery root f *> return {}
    sub <- R.subscribeComputed pred $ \b ->
      render $ if b then onTrue else onFalse
    k sub)
